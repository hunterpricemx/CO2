...
function EditDownloadForm() {
  ...
  const {
    ...
    refineCoreProps: { resource: "downloads", action: "edit", id, redirect: "list" },
  });
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-bebas text-4xl tracking-wider text-white">Editar Descarga</h1>
      </div>
      ...
      >
        ...
        <div>
          ...
          <input {...register("url", { required: true })} className={FIELD_CLS} />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={LABEL_CLS}>Tamaño del archivo</label>
            <input {...register("file_size")} className={FIELD_CLS} placeholder="2.4 GB" />
          </div>
          <div>
            <label className={LABEL_CLS}>Orden</label>
            <input {...register("sort_order", { valueAsNumber: true })} type="number" className={FIELD_CLS} />
          </div>
        </div>
