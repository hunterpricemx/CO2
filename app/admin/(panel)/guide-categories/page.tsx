...
import type { GuideCategoryRow } from "@/modules/guides/types";
...
export default function AdminGuideCategoriesPage() {
  ...
  const categories = (query.data?.data ?? []) as GuideCategoryRow[];
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-bebas text-5xl tracking-wider text-white">Categorias de guias</h1>
          <p className="text-sm text-gray-500 mt-1">{categories.length} categorias registradas</p>
        </div>
        <Link
          href="/admin/guide-categories/create"
                ...
                <tr key={c.id} className="border-b border-[rgba(255,255,255,0.04)] hover:bg-white/2">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2 justify-end">
                      <Link href={`/admin/guide-categories/edit/${c.id}`} className="p-1.5 text-gray-400 hover:text-[#f39c12] hover:bg-[rgba(243,156,18,0.1)] rounded-lg transition-colors">
                        <Pencil className="h-3.5 w-3.5" />
                      </Link>
                      <button
                        onClick={() => {
                          if (confirm(`¿Eliminar categoria "${c.name_es}"?`)) {
                            remove({ resource: "guide_categories", id: c.id });
                          }
                        }}
