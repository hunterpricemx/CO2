...
import { updateAccesory } from "../../actions";
...
export default function EditAccesoryPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error || !data) {
          toast.error("Accesory no encontrado.");
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await updateAccesory(id, {
        name,
        description,
        image_url: imageUrl,
        active,
      ...
      if (result.success) {
        toast.success("Accesory actualizado.");
        router.push("/admin/accesory");
      } else {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center py-20"><Loader2 className="h-6 w-6 text-[#f39c12] animate-spin" /></div>;
  }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-bebas text-4xl tracking-wider text-white">Editar Accesory</h1>
      </div>

      <form onSubmit={handleSubmit} className="bg-[#1a1a1a] border border-[rgba(255,215,0,0.1)] rounded-xl p-6 flex flex-col gap-5">
        <div><label className={LABEL_CLS}>Nombre *</label><input value={name} onChange={(e) => setName(e.target.value)} className={FIELD_CLS} required /></div>
        <div><label className={LABEL_CLS}>Descripcion</label><textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className={`${FIELD_CLS} resize-none`} /></div>
        <ImageUploadField label="Imagen del accesory" value={imageUrl} onChange={setImageUrl} folder="accesory" allowVideo />
        <div><label className={LABEL_CLS}>Orden de aparicion</label><input type="number" value={sortOrder} onChange={(e) => setSortOrder(Number(e.target.value))} className={FIELD_CLS} min={0} /></div>
        ...
        <button type="submit" disabled={isPending} className="flex items-center justify-center gap-2 bg-[#f39c12] hover:bg-[#e67e22] disabled:opacity-50 text-black font-semibold px-4 py-2.5 rounded-lg transition-colors">{isPending && <Loader2 className="h-4 w-4 animate-spin" />}{isPending ? "Guardando..." : "Guardar Cambios"}</button>
      </form>
    </div>
  );
}
