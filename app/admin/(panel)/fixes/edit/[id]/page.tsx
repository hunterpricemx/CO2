...
export default function FixEditPage() {
  const back = useBack();
  const {
    register,
    control,
    ...
    refineCoreProps: { resource: "fixes", action: "edit", redirect: "list" },
  });
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-bebas text-4xl tracking-wider text-white">Editar Fix</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onFinish)} className="bg-[#1a1a1a] border border-[rgba(255,215,0,0.1)] rounded-xl p-6 flex flex-col gap-5">
        ...
        <div>
          <label className={LABEL_CLS}>URL de Video</label>
          <input {...register("video_url")} className={FIELD_CLS} />
        </div>
