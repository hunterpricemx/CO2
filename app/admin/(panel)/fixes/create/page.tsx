...
import type { FixCategoryRow, FixFormData } from "@/modules/fixes/types";
...
function FixForm({ action }: { action: "create" | "edit" }) {
  const back = useBack();
  const {
    register,
    control,
    ...
    refineCoreProps: { resource: "fixes", action, redirect: "list" },
    defaultValues: action === "create" ? { status: "draft", version: "both" } : undefined,
  });

  const thumbnail = watch("featured_image");
  const { query: categoriesQuery } = useList<FixCategoryRow>({
    resource: "fix_categories",
    sorters: [{ field: "sort_order", order: "asc" }],
    pagination: { pageSize: 100 },
  });
  const categories: FixCategoryRow[] = categoriesQuery.data?.data ?? [];
    ...
    <div className="flex flex-col gap-6 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-bebas text-4xl tracking-wider text-white">
          {action === "create" ? "Nuevo Fix" : "Editar Fix"}
        </h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onFinish)} className="bg-[#1a1a1a] border border-[rgba(255,215,0,0.1)] rounded-xl p-6 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(["es", "en", "pt"] as const).map((lang) => (
            <div key={lang}>
              ...
              <input {...register(`title_${lang}`, { required: true })} className={FIELD_CLS} />
            </div>
          ))}
        </div>
        ...
        <div>
          <label className={LABEL_CLS}>URL de Video (opcional)</label>
          <input {...register("video_url")} className={FIELD_CLS} placeholder="https://youtube.com/watch?v=..." />
        </div>
        ...
        <div className="flex justify-end gap-3 pt-2 border-t border-[rgba(255,255,255,0.06)]">
          <button type="button" onClick={back} className="px-4 py-2 text-sm text-gray-400 hover:text-white border border-[rgba(255,255,255,0.1)] rounded-lg transition-colors">Cancelar</button>
...
export default function FixCreatePage() {
  return <FixForm action="create" />;
}
