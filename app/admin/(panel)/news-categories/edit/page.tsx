...
function EditNewsCategoryForm() {
  ...
  const {
    ...
    refineCoreProps: { resource: "news_categories", action: "edit", id, redirect: "list" },
  });
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(["es", "en", "pt"] as const).map((lang) => (
            <div key={lang}>
              ...
              <input {...register(`name_${lang}` as "name_es" | "name_en" | "name_pt", { required: true })} className={FIELD_CLS} />
            </div>
          ))}
        </div>
        <div>
...
export default function NewsCategoryEditPage() {
  return (
