...
function AccesoryMedia({ url, name }: { url: string | null; name: string }) {
  if (!url) return null;
...
function Card({
  ...
  const message = [
    "Hola, quiero solicitar este Accesory.",
    `Accesory ID: ${item.id}`,
    `Accesory: ${item.name}`,
    `Version: ${version}`,
  ...
  return (
    <div className="group flex flex-col rounded-2xl overflow-hidden" style={{ background: "rgba(15,5,3,0.92)", border: "1px solid rgba(255,215,0,0.13)" }}>
      <div className="relative h-52 bg-black overflow-hidden">
        {item.image_url ? (
          <>
            <AccesoryMedia url={item.image_url} name={item.name} />
...
export function AccesoryClient({ items, version, whatsappPhone }: { items: AccesoryItem[]; version: string; whatsappPhone: string }) {
    ...
    <>
      ...
      <section className="mb-10">
        ...
        {paginatedAvailable.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-3 rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#120805]">
            ...
            <p className="text-sm text-gray-500">No hay accesory disponibles con este filtro.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginatedAvailable.map((item) => (
              <Card key={item.id} item={item} version={version} phone={whatsappPhone} reserved={false} buttonText="Solicitar Accesory" onZoom={(url, name) => { setZoomUrl(url); setZoomName(name); }} />
            ))}
          </div>
        )}
      ...
      <section className="mb-10">
        ...
        {paginatedReserved.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-3 rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#120805]">
            <ShoppingBag className="h-10 w-10 text-gray-700" />
            <p className="text-sm text-gray-500">No hay accesory apartados con este filtro.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginatedReserved.map((item) => (
              <Card key={item.id} item={item} version={version} phone={whatsappPhone} reserved buttonText="Consultar Accesory" onZoom={(url, name) => { setZoomUrl(url); setZoomName(name); }} />
            ))}
          </div>
        )}
